...
Companions: [`ARCHITECTURE_PLAN.md`](ARCHITECTURE_PLAN.md) Phase 2 (the source material — see §2.1; the referenced external plan file was confirmed dead by Matt, 2026-09-23 — this in-repo document is canonical), [`ENGINE_PLAN.md`](ENGINE_PLAN.md) (structural/tonal model for this doc), [`SHELL_BRAND.md`](SHELL_BRAND.md), [`TWO_PLANES.md`](TWO_PLANES.md), [`EMBARGO.md`](EMBARGO.md), [`AGENTS.md`](../AGENTS.md).
...
| **Phase 4 (PLAY: create the panel; EVOLVE + beat router)** | `panels/PlayPanel.jsx` (new), `PanelRegistry.js` (`play` entry, alongside still-live `davis`/`stimulus`), `panels/DavisPanel.jsx` (trimmed) | Shipped 2026-09-22, PR #447 (`feat(ui): PLAY panel — EVOLVE chips + shimmer + beat router (#248 Phase 4)`). `EvolveControls.jsx`/`BeatRouter.jsx` stay in `panels/davis/`, cross-imported. `PlayPanel.jsx` computes its own `beatCollision` from a state slice split across DAVIS+PLAY — verified live that the cross-panel gate still works. `tour.js` step 3's now-half-stale "DAVIS tab" body text left alone on purpose (its accurate fix only holds once Phase 5 lands too; that's still Phase 12's job). §3's Phase 4 section is historical record; do not redo it. |
| **Phase 5 (PLAY: move MORPH EVOLVE + PHRASE LOOP)** | `panels/PlayPanel.jsx`, `panels/DavisPanel.jsx` (trimmed further), `panels/davis/MorphControls.jsx` + `PhraseControls.jsx` (moved, not rewritten), `helpCopy.js` (`davis-morph` group relabeled) | Shipped 2026-09-23, PR #497 (`feat(ui): PLAY absorbs MORPH EVOLVE + PHRASE LOOP (#248 Phase 5)`). Verified live: PLAY shows both sections, GHOST STATION no longer does, the MORPH EVOLVE toggle is fully wired in its new home. `DavisPanel.jsx` intentionally keeps the phrase/morph state selectors its own header subtitle still reads — cross-panel status at a glance, same precedent Phase 4 set for `beatCollision`. `tour.js` untouched, still Phase 12's job. §3's Phase 5 section is historical record; do not redo it. Phase 6 (PLAY: move transport buttons + sub-seed stream mutators) is the next open phase. |
...
1. **The plan file the issue points at does not exist here.** `~/workspace/your_files/ui-simplify-plan/teenage-engineering-ui-plan.md` is outside this repo and was not present in this environment. This document was built from `docs/ARCHITECTURE_PLAN.md` Phase 2 (which independently describes the identical 4-tab map, so the two are very likely the same plan) plus a direct code inventory. **If the external file exists elsewhere and says something different in the details, reconcile before executing — do not assume this document silently supersedes it.** **RESOLVED (Matt, 2026-09-23): the external file does not exist and will not be chased — this document is canonical.**
...
### Phase 3 — BUILD: absorb LAYERS — **SHIPPED, PR #445**
...
### Phase 4 — PLAY: create the panel; move EVOLVE chips + shimmer + beat router — **SHIPPED, PR #447**
...
### Phase 5 — PLAY: move MORPH EVOLVE + PHRASE LOOP — **SHIPPED, PR #497**

    ...
    <div className="fx-editor" title="Effect stack">
        ...
        return (
          <div className="fx-effect" key={`${fx.kind}-${i}`}>
            <div className="fx-effect-head">
              ...
              <button className="micro-btn" disabled={i === 0} title="Move effect earlier"
                onClick={() => emit(Events.FX_EFFECT_REORDER, { layerId: layer.id, index: i, delta: -1 })}>▲</button>
              <button className="micro-btn" disabled={i === effects.length - 1} title="Move effect later"
                onClick={() => emit(Events.FX_EFFECT_REORDER, { layerId: layer.id, index: i, delta: 1 })}>▼</button>
      ...
      <div className="fx-add-row">
        <select className="tg" value={addKind} onChange={(e) => setAddKind(e.target.value)}>
          {FX_MENU_KINDS.map((k) => (
            <option key={k} value={k}>{FX_EFFECT_DEFS[k].label.toUpperCase()}</option>
          ))}
        </select>
  ...
  return (
    <div className="panel panel-layers">
      <PanelHeader tag="P08" title="LAYERS" subtitle={`${contentCount} / ${MAX_CONTENT_TRACKS} tracks`}>
        <button className="chip-btn" disabled={contentCount >= MAX_CONTENT_TRACKS}
          onClick={() => emit(Events.LAYER_ADD)}>+ ADD LAYER</button>
        <button className="chip-btn" disabled={fxCount >= MAX_FX_TRACKS}
          onClick={() => emit(Events.LAYER_ADD_FX)}>+ ADD FX</button>
      </PanelHeader>
      <div className="panel-body layer-list">
        {ghosts.slice().reverse().map((n) => (
          <div key={`ghost-kc-${n}`} className="layer-row" style={{ opacity: 0.35 }}
            title={`Tap to arm KC-${n}`} onClick={() => emit(Events.LAYER_ADD)}>
          ...
          return (
            <div key={layer.id} className={`layer-row ${isActive ? 'layer-row-active' : ''} ${fx ? 'layer-row-fx' : ''} ${isFxSelected ? 'layer-row-fx-selected' : ''}`}>
              <div className="layer-row-main">
                ...
                <button className="micro-btn" onClick={() => emit(Events.LAYER_TOGGLE_VISIBLE, { id: layer.id })}>
                  {layer.visible ? '●' : '○'}
                </button>
                <button className="micro-btn" onClick={() => emit(Events.LAYER_SOLO, { id: layer.id })}>
                  {soloed ? 'S·' : 'S'}
                </button>
                ...
                <button className="micro-btn" disabled={layers.length <= 1}
              ...
              <div className="layer-row-composite">
                {fx ? (
                  ...
                  <select className="tg blend-mode-select" value={layer.layerBlendMode}
              ...
              {!fx && (
                <div className="layer-row-composite" title="PATCH — FEED amount is the extra slider">

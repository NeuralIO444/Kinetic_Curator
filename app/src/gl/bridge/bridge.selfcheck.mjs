...
// roster's Blur entry is dead on the GPU path until #310 cuts it from the
// UI: a chain carrying kind 'blur' fails closed with "unknown effect kind",
// exactly like the Phase-2 kinds. The honest-blur subdivision tests below
// were deleted with the blur passes.)
ok('blur is dead on the GPU path: kind "blur" fails closed (#308)', () => {
  const gl = makeMockGl({ nullUniforms: ['u_res'] });
  const bridge = makeBridge(gl);
  const L = bridge.layer('fx1');
  assert.throws(
    () => bridge.runChain('fx1', L.t0, [{ kind: 'blur', params: { radius: 6 } }]),
    /unknown effect kind "blur"/,
    'the FX roster Blur entry throws at chain time until #310 cuts it',
  );
  // The four surviving builtins still chain top-down.
  const out = bridge.runChain('fx1', L.t0, chainOf('invert', 'rgbSplit', 'grain', 'posterize'));
  assert.equal(out, L.t0, 'chain returns the final read target');
  const modes = gl.withName('uniform1i').filter((c) => c.args[0] === 'u_effect').map((c) => c.args[1]);
  assert.deepEqual(modes, [0, 1, 2, 5], 'invert, rgbSplit, grain, posterize — no blur ids');
});
